...
#     valueslist = list(area.values())
#     valuesIndex = valueslist.index(cityName)
#     keysList = list(area.keys())
#     key = keysList[valuesIndex]
#     return key
class ZzSpider(object):
    def __init__(self,cityList = [],jobName=''):
        ...
        for cityName in self.cityList:
            if cityName == self.cityList[:-1]:
                self.cityString +=cityName
            else:
                self.cityString += cityName
                self.cityString+='%252C'
    def get_code_with_url(self,page):
        full_url = self.base_url+self.cityString+',000000,0000,00,9,99,'+self.jobName+',2,'+str(page)+'.html'
        print(full_url)
    ...
    def get_total_page(self,code):
        pattern = re.compile(r'<div class="dw_page".*?<span class="td">共(.*?)页.*?</span>',re.S)
        result = pattern.findall(code)
        self.total_page =  int(result[0])
    def get_all_info(self,code):
        pattern = re.compile(r'<div class="el">.*?<a target="_blank".*?>(.*?)</a>.*?<span class="t2">.*?<a.*?title="(.*?)".*?</a>'
                             r'.*?<span class="t3">(.*?)</span>.*?<span class="t4">(.*?)</span>',re.S)
        result= pattern.findall(code)
        # print(result)
        list = []
        for value in result:
            ...
            list.append([jobName,companyName])
        return list
    ...
    def start_spider(self):
        ...
        for index in range(1,self.total_page+1):
            ...
            if code ==None:
                continue
            result_list =self.get_all_info(code)
            for job, company in result_list:
...
jobName = input('请输入你从事的工作')
QTWY = ZzSpider(cityList,jobName)
QTWY.start_spider()
